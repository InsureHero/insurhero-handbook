...
| **Titular** | Constantes de tipo holder y titular de póliza acordes al modelo AMA (`AMA_HOLDER_TYPE`, `AMA_POLICY_HOLDER`). El flag booleano `holder.policyHolder` va **siempre en `false`** (`AMA_POLICY_HOLDER = false`) para todo beneficiario, sea o no el tomador — AMA no presta el servicio al tomador como tal, así que el flag no refleja el rol de negocio. No confundir con el objeto raíz `policyHolder` (ver **Beneficiarios**). |
| **Beneficiarios** | Mapeo explícito `mapBeneficiariesToAma` desde el risk item — un `AmaCreateHolderRequest` por beneficiario. El rol de tomador (`isHolder`) sigue gobernando el resto del mapeo: solo cuando el beneficiario es el tomador se agregan `type`/dirección al `holder` y se incluye el objeto raíz `policyHolder` con sus datos (`name`, `surname1`, `surname2`). |
